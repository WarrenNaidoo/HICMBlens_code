#########################################################################################################################
# In this module we copmute all the z-bin properties required for our computations in each redshift bin.
# Here given a binning strategy defined in the settings.py file the code computes each of the bins, taking
# the bin centers, the widths and computes the corresponding ell,y modes as well as the comoving distance, volume and
# mode volume factor of the bin used in the Fisher matrix calculation.
#########################################################################################################################
...
C_Mpc=3e5*(3.24078e-20) # Speed of light Mpc/s
...
class ZBinProp:
    def __init__(self, zbin_index, zbinning_struct, expt):
      ...
      self.y_min_expt = 0.01*self.rnu_A#settings.mode_factor_y/self.Deltanutilde #(Taking car of foreground cut)
      #self.y_min_expt = settings.mode_factor_y/self.Deltanutilde #(Taking car of foreground cut)
...
class ZBinningStruct:
    def __init__(self, expt):
        ...
        if settings.Z_BIN_FORMAT == 'LINEAR_100MHz_BINS':
            ...
            self.min_nutilde_arr = expt.numin/(1.e6*settings.nu21) + self.nu_bin_width/settings.nu21 * np.arange(8)
