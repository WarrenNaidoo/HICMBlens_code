...
def logpMv_derivative(k_in):
    kh,P = np.loadtxt('dlnP_dMv_z0_4.dat')
    k_arr = kh/cosmological_parameters.h
    dlog_interp = interp1d(k_arr, P,bounds_error=False,fill_value=0.0)
    return dlog_interp(k_in)

def logpMv_derivative_2(k_in):
    kh,P = np.loadtxt('dPm_dnu_Omega_m_029.dat') #    dPm_dnu.dat
...
def CMB_convergence_power(ells, show_plots=False):
    #CMB signal and noise (signal not used, just for plot)
...
#########################################################################################
#########################################################################################
############## DERIVATIVES FOR FISHER MATRICES
#########################################################################################
#########################################################################################
...
def Cl21_Distance_derivs(zbin_prop,ell_arr, y_arr,fid_params, bias_var=1., gamma_var=1.):
   ...
   dC_ell_dMV = (logpMv_derivative_2(ktot_2d_arr)/P_spl_k(ktot_2d_arr))*Cl_21_auto_ell_y_2d_arr

   return  dC_ell_y_dAbao, dC_ell_y_dsig8_zA, dC_ell_y_db1_zA, dC_ell_y_db2_zA, dC_ell_dMV, deriv_f, deriv_aperp, deriv_apar


def Cl_kappa_distance_derivs(ell_arr, y_arr, zbin_prop, fid_params, bias_var=1., gamma_var=1.): # 21cm angular PS at z vs ell,y
    ...
    Cl_kappa_auto_ell_y_2d_arr = CMB_convergence_power(kperp_2d_arr*chi_A)
   ...
   ### For nonlinear cases sigma_nl must add extra terms dlogpk_dk + drsd_dk + dbias_k below as in Bull et al.
    ...
    dC_ell_y_dmnu = Cl_kk_dmnu_f1(kperp_2d_arr*chi_A)
...
def bispec_distance_derivs(ell_coarse_arr, y_coarse_arr, zbin_prop, fid_params, bias_var=1., gamma_var=1.):
    ...
    Int_alt_kavi_since_max = np.sin(np.amax(qperp_arr)*delta_chipar_bin/2.0)/ (np.amax(qperp_arr)*delta_chipar_bin/2.0)
    Int_alt_kavi_since_min = np.sin(qpar_min*delta_chipar_bin/2.0)/ (qpar_min*delta_chipar_bin/2.0)
    Int_alt_kavi_Pm_min = power_spec_functions.get_P_m_0(qpar_min)
    Int_alt_kavi_Pm_max = power_spec_functions.get_P_m_0(np.amax(qperp_arr))
    ...
    deriv_Kavi_aperp = np.amax(qperp_arr)**2. * Int_alt_kavi_since_max**2. * Int_alt_kavi_Pm_max*daperp_q_at_qmax - qpar_min**2. *Int_alt_kavi_since_min**2. * Int_alt_kavi_Pm_min*daperp_q_at_qmin
    deriv_Kavi_apar = np.amax(qperp_arr)**2. * Int_alt_kavi_since_max**2. * Int_alt_kavi_Pm_max*dapar_q_at_qmax - qpar_min**2. *Int_alt_kavi_since_min**2. * Int_alt_kavi_Pm_min*dapar_q_at_qmin
    ...
    Integral_ell_y_zA_over_qpar_Mv = np.trapz(0.5/np.pi**2 * logpMv_derivative(q_mag_2d_arr) *K_21_qperp_2d_arr*K_kappa_qperp_2d_arr*K_21_qpar_2d_arr*K_kappa_qpar_2d_arr*q_perp_2d_arr, x=qperp_arr, axis=0)
    Integral_ell_y_zA_over_qpar_qperp_Mv = np.trapz(Integral_ell_y_zA_over_qpar, x=qpar_arr, axis=0)


    Soln_Kavi_aperp = (1/2.*np.pi)**2. *  deriv_Kavi_aperp/Integral_ell_y_zA_over_qpar_qperp
    Soln_Kavi_apar = (1/2.*np.pi)**2. *  deriv_Kavi_apar/Integral_ell_y_zA_over_qpar_qperp
   ...
   # Now evaluate all terms in bispectrum
    ...
    deriv_aperp = ( (4./aperp) + deriv_P21_aperp_over_P21  + Soln_Kavi_aperp\
    ...
    deriv_apar =   ( (1./apar) + deriv_P21_apar_over_P21  + Soln_Kavi_apar\
    ...
    dB_ell_y_Mv = ((logpMv_derivative_2(ktot_2d_arr)/power_spec_functions.get_P_m_0(ktot_2d_arr))/P_21_z0_2d_arr + (Integral_ell_y_zA_over_qpar_qperp_Mv/Integral_ell_y_zA_coarse ) )  *C_ell_y_zA
...
# ABOVE ARE LEVEL 1 MODULES

#####################################

# BELOW ARE LEVEL 2 MODULES



#####################################################################################################################
#####################################################################################################################
######################## SIGNAL-TO-NOISE AND FISHER MATRICES
#####################################################################################################################
#####################################################################################################################
...
def compute_SNR_Cl_21_kappa_cross(zbin_prop, k_min = 1.e-7, k_max = 3.0e0,
    ...
    y_min_full = k_min * rnu_A ; y_max_full = k_max * rnu_A # to capture oscillations - LINK TO W_kappa kmax
...
def get_three_param_FisherMatrix_Cl_21_21_auto(zbin_prop, num_k_bins_ell = 15, num_k_bins_y=14):
    ...
    Cl_Abao, Cl_sig8, Cl_b1, Cl_b2, Cl_Mv, Cl_f, Cl_aperp, Cl_apar = Cl21_Distance_derivs(zbin_prop, ell_arr_ltd, y_arr_ltd, fid_fisher_params, bias_var=1., gamma_var=1.)
...
def expand_fisher_matrix(zbin_prop, derivs, F, names, exclude=[]):
	# Based on the code from BAO_Fisher from Phil Bull et al. arXiv:1405.1452
    """
...
def get_Clkappa_distance_params_fisher(zbin_prop, num_k_bins_ell = 15, num_k_bins_y=14):
    ...
    Cl_sig8, Cl_Mv, Cl_Om_DE, Cl_w0, Cl_wa, Cl_h = Cl_kappa_distance_derivs(ell_arr_ltd, y_arr_ltd, zbin_prop, fid_fisher_params, bias_var=1., gamma_var=1.)

    Cl_signal = CMB_convergence_power(ell_arr_ltd)

    parameter_deriv_C_ell_y_arr = np.array([Cl_sig8, Cl_Mv, Cl_Om_DE, Cl_w0, Cl_wa, Cl_h])  #,Cl_Omega_HI

    ell_2d_arr_ltd=np.outer(ell_arr_ltd,np.ones(n_y))

    Cl_kappa_noise_ell_arr=CMB_convergence_noise(ell_arr_ltd)
...
def compute_SNR_Bl_21_21_kappa_cross(zbin_prop, delta_kpar = 0.01, delta_kperp = 0.01,
    ...
    n_ell_coarse = 999 ; n_y_coarse = 1000  ## CAN GO UP TO 200 -249 possible memory issue if go larger
    ...
    plt.savefig('Warren_new_plots_output/'+'Bispectrum_spectra.png',dpi=300)
    ...
    plt.savefig('Warren_new_plots_output/'+'Bispectrum_bin'+str(round(z_A,2))+'bin.png',dpi=300)
...
def get_distance_params_bispec_fisher(zbin_prop, num_k_bins_ell = 15, num_k_bins_y=14):
    ...
    Cl_21_auto_ell_y_2d_arr=Cl_21_auto_ell_y_zA(ell_arr, y_arr, zbin_prop, bias_var=1., gamma_var=1.) # 21cm angular PS at z vs ell,y
    Cl_21_noise_ell_arr=HI_angular_noise_ell_y(ell_arr, zbin_prop, experiment_name='hirax', mode='interferometer', show_plots=False)
    Cl_21_noise_ell_y_2d_arr=  np.outer(Cl_21_noise_ell_arr,np.ones(n_y))
    ...
    Bl_Abao, Bl_sig8, Bl_b1, Bl_b2, Bl_Mv, Bl_f, Bl_aperp, Bl_apar =  bispec_distance_derivs(ell_arr_ltd, y_arr_ltd, zbin_prop, fid_fisher_params, bias_var=1., gamma_var=1.)

    Bl_signal = integ_bispec_kappa_21_21_ell_y_zA(ell_arr_ltd, y_arr_ltd, zbin_prop, bias_var=1., gamma_var=1.)
    ...
    ell_2d_arr_ltd=np.outer(ell_arr_ltd,np.ones(n_y))
    CMB_kappa_noise_ell = CMB_convergence_noise(ell_2d_arr_ltd)
    CMB_kappa_signal_ell = CMB_convergence_power(ell_2d_arr_ltd)
