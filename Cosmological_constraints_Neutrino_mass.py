...
# Fisher matrices for the Neutrino masss- w0waCDM case and used the Full21cmblens_primary_functions.py module to plot
# the results.
...
for i in range(0,4):
    ...
    fid_label, fid_values, fisher_3params_21 = bl.get_three_param_FisherMatrix_Cl_21_21_auto_Neutrino_mass(zbin_prop)
    fid_label_kappa, fid_values_kappa, fisher_kappa = bl.get_Clkappa_distance_params_fisher_Neutrino_mass(zbin_prop)
    fid_label_bispec, fid_values_bispec, fisher_bispec = bl.get_distance_params_bispec_fisher_Neutrino_mass(zbin_prop)
...
# Print erros on combined Fisher matrix before and after removing w0-wa to see effect on neutrino names_params_bispec

print('error on Neutrino mass sum with w0-wa', np.sqrt(np.diag(np.linalg.inv(Fpl_comb))))

# Fix w0-wa
Fpl_comb  = np.delete(np.delete(Fpl_comb, [2,3],axis=1 ), [2,3],axis=0)

print('error on Neutrino mass sum fixed w0-wa', np.sqrt(np.diag(np.linalg.inv(Fpl_comb))))
